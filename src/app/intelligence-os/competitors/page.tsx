...
import {
  ...
  ExternalLink, Menu, GripVertical,
} from 'lucide-react';
...
const MOCK_COMPETITORS = [
  ...
  },
  {
    ...
    threats: [
      { type: 'feature', label: 'Nova feature de AutoML', date: '3 dias atrás', severity: 'high' },
    ],
  ...
  },
  {
    ...
    threats: [
      { type: 'feature', label: 'Integração com TikTok Ads', date: '1 dia atrás', severity: 'medium' },
    ],
...
export default function CompetitorsPage() {
  const [selectedCompetitor, setSelectedCompetitor] = useState(MOCK_COMPETITORS[0]);
  const [showModal, setShowModal] = useState(false);
  type SwotKey = keyof typeof selectedCompetitor.swot;
  const [swotItems, setSwotItems] = useState(() =>
    SWOT_CATEGORIES.map((cat) => ({
      category: cat.key,
      items: [...selectedCompetitor.swot[cat.key as SwotKey]],
    })),
  );

  const handleCompetitorChange = (id: string) => {
    const comp = MOCK_COMPETITORS.find((c) => c.id === id);
    if (comp) {
      setSelectedCompetitor(comp);
      setSwotItems(
        SWOT_CATEGORIES.map((cat) => ({
          category: cat.key,
          items: [...comp.swot[cat.key as keyof typeof comp.swot]],
        })),
      );
    }
  };
    ...
    >
      ...
      >
        <div>
          ...
          <p className="mt-2" style={{ color: COLORS.muted }}>
            Monitore, analise e supere seus concorrentes com análises profundas de mercado
          </p>
        </div>
        <motion.button
      ...
      <section className="mb-16">
        ...
        <div className="grid gap-4 md:grid-cols-2">
          {SWOT_CATEGORIES.map((cat, catIdx) => {
              ...
              <GlassCard key={cat.key} className="p-5">
                ...
                <Reorder.Group
                  ...
                  onReorder={(newOrder) => {
                    setSwotItems((prev) =>
                      prev.map((s) =>
                        s.category === cat.key ? { ...s, items: newOrder } : s,
                      ),
                    );
                  }}
      ...
      <section className="mb-16">
        <SectionTitle
          ...
          subtitle="Comparação direta das principais métricas"
        />
            ...
            <tbody>
              {MOCK_COMPETITORS.map((comp, idx) => (
                ...
                >
                  ...
                  <td className="py-4 px-3 font-medium" style={{ color: COLORS.accent }}>
                    {comp.price}
                  </td>
      ...
      <section className="mb-16">
        <SectionTitle
          icon={Search}
          title="Análise de Estratégia Digital"
          subtitle="SEO, Ads, Conteúdo e Social Media — score consolidado"
        />
      ...
      <section className="mb-16">
        <SectionTitle
          ...
          subtitle="Posicionamento Preço × Qualidade"
        />
        <GlassCard className="relative overflow-hidden p-4 md:p-6">
          ...
          <div
            className="relative h-[320px] w-full overflow-hidden rounded-xl border"
            style={{
              background: `linear-gradient(135deg, ${COLORS.surface} 0%, #0a0e1a 100%)`,
              borderColor: COLORS.glassBorder,
            }}
          >
            {/* Grid lines */}
            ...
            {/* Center crosshair */}
            <div
              className="absolute left-1/2 top-1/2 h-16 w-16 -translate-x-1/2 -translate-y-1/2 rounded-full opacity-10"
              style={{ background: COLORS.accent }}
            />
            {/* Quality label */}
            <div
              className="absolute -right-24 top-1/2 -mt-20 origin-center -rotate-90 text-xs tracking-widest"
      ...
      <section className="mb-16">
        <SectionTitle
          ...
          subtitle="Alertas de mudanças nos concorrentes"
        />
        <div className="space-y-3">
          {MOCK_COMPETITORS.flatMap((comp) =>
            comp.threats.map((threat, idx) => {
              ...
              return (
                ...
                >
                  <GlassCard
                    className="flex items-center gap-4 p-4 transition-all hover:border-[#3DF5C5]/20"
                  >
                    <div
      ...
      <section className="mb-16">
        <SectionTitle
          ...
          subtitle="Distribuição de menções na web"
        />
        <div className="grid gap-6 md:grid-cols-2">
          <GlassCard className="p-6">
            <ResponsiveContainer width="100%" height={280}>
              <PieChart>
                <Pie
                  data={MOCK_COMPETITORS.map((c) => ({ name: c.name, value: c.shareOfVoice }))}
                  cx="50%"
                  cy="50%"
                  innerRadius={60}
                  outerRadius={100}
      ...
      <section className="mb-16">
        <SectionTitle
          icon={Zap}
          title="Gap Analysis"
          subtitle="Onde seu negócio pode se destacar"
        />
      ...
      >
        <p className="flex items-center justify-center gap-2">
          ...
          Competitive Intelligence Platform · Atualizado em tempo real
        </p>
      </motion.footer>
    </main>
  );
